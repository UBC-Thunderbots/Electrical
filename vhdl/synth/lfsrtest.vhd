...
architecture Behavioural of LFSRTest is
...
begin
	...
	-- The LFSR.
	LFSRIntance : entity work.LFSR10(Behavioural)
	port map(
		Clock => Clock1,
		Shift => LFSREnable,
		Reset => '0',
		...
		if rising_edge(Clock1) then
			...
			if State = Init then
				...
				if WriteNextCycle then
					...
					if WritePWMs then
						if LFSROut = '1' then
							RAMWData <= to_signed(HighSpeed, RAMWData'length);
						else
							RAMWData <= to_signed(LowSpeed, RAMWData'length);
						end if;
					else
