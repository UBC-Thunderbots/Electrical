...
namespace {
	...
	const instruction_pattern ALL_INSTRUCTIONS[] = {
		instruction_pattern("ADDS", instruction_pattern::ARG_RA_RW, instruction_pattern::ARG_RB_RO, 0),
...
int main() {
	...
	std::cout << "\ttype IOInType is array(0 to 31) of signed(15 downto 0);\n";
	...
	std::cout << "\tIOInData <= IOIn(to_integer(IOAddress));\n";
	std::cout << '\n';
	if (!p.get_oport_names().empty()) {
		...
		for (unsigned int i = 0; i < p.get_oport_names().size(); ++i) {
			if (i == 0) std::cout << "\t\t\t\tif IOAddress = to_unsigned(" << i << ", 5) then\n";
			else std::cout << "\t\t\t\telsif IOAddress = to_unsigned(" << i << ", 5) then\n";
